        ...
        for g in range(self.config.get('num_generations')):
            ...
            if g == 0:
                ...
                if self.config.get('increase_beta') != 1:
                    if g == 1:
                        self.config.set('offset_epoch',
                                        self.config.get('init_num_epochs'))  # offset epoch for computing beta
                    else:
            ...
            # create a sampler for sampling from random noise or sampling from modified z
            self.sampler = Sampler(self.config, self.vocab, self.trainer.model)

            if (self.population_size <= 1000 and g < self.config.get('num_generations')) or (
                    self.population_size > 1000 and g in [0, math.ceil(self.config.get('num_generations') / 2) - 1,
                ...
                if num_samples > 30000:
                    num_samples = 30000
                samples_random, valids_random = self.sampler.sample(num_samples=self.population_size,
                                                                    save_results=False,
